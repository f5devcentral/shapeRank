...
The effect of $l$ is that variable $ID$ is added to the enclosing program's compile-time scope.
...
A program signature specifies the name of the program as whole. In addition it specifies each of the program's parameters.  Each parameter has a specifiation similar to that of a function parameter;. However, rather than specifying a rank for each parameter, a shape and base type are specified. The shape may be elided. Unlike a function, a program is not invoked by a function call [\ref{calls}]. Instead, it is invoked from the surrounding computing environment in an implementation specific manner.  The invocation binds the parameters declared in the program signature to external data sources. These sources are the program's sole source of input. The program's result is the value of its expression, and this is its sole external output. 
...
divergence. The abstract interpretation is independent of the size of the data the program operates on (except for literal arrays).

The approach has significant advantages and disadvantages, but we feel the advantages greatly outweigh the disadvantages.

First, programmers need not specify shape and/or dimension variables in signatures. Second, typing is actually more accurate.
} 
...
For parameters representing external functions, the base type is  function type where each parameter has a complete type signature including rank, dimensions and base type.
