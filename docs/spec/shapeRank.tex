...
It is easiest to understand the process by considering a function of one argument, such as logical negation, $not()$. Given a call such

$not([true, false])$

we see that the rank of the argument is 1, but the function expects a scalar argument, i.e, an argument of rank 1. We therefore produce a frame of rank $1 - 0 = 1$, with cells of rank $0$.  We can now invoke $not()$ on each of the two cells, and produce a result by assembling the scalar results into a result vector, $[false, true]$.
...
To deal with these, we need the notion of common frame for all the arguments as described below.
}
...
An {\em integer literal} represents an integer scalar.  Integer literals have type $[]\INT{}$ in any scope $S$.
...
The type of a string literal in any scope $S$ is $[]\STRING$.
...
{\it
This arrangement is unusual. It means that an expression is typechecked with respect to the types of the actual arguments of the surrounding function rather  than their
