...
ShapeRank is a purely functional, decidable, rank-polymorphic statically shape-safe language. It is designed for data analysis, reactive programming and machine learning. 
...
The most elementary hyperstreams are {\em scalar hyperstreams}. These include  booleans [\ref{bool}], numbers [\ref{number}], strings [\ref{string}] and  structs [\ref{structTypes}]. A scalar hyperstream consists of a single scalar value, and has zero dimensions, hence its shape is the empty sequence, $[]$. 

{\em The rank of a scalar hyperstream is therefore $0$.}

 We will often refer to scalar hyperstreams simply as scalars.
...
{\em function signature} which specifies the name of the function, and the rank of each of its parameters.
...
A function signature has the form  $F\LPAREN{}p_1\COLON R_1\COMMA{} \ldots\COMMA{} p_n\COLON R_n\RPAREN{}$ where $F$ is an identifier that specifies the name of the function, and $p_i\COLON{} T_i, i \in 1..n$ specify the function's parameters; each $p_i$ is an identifier that names the $i$th parameter, and the corresponding $R_i$ specifies the parameter's rank.  The specification of a parameter's rank may be omitted in which case it defaults to 0.

It is a compile-time error if $p_i = p_j, i \ne j$.

Let $f \triangleq$ \FUNC{} $F\LPAREN{}p_1\COLON R_1\COMMA{} \ldots\COMMA{} p_n\COLON R_n\RPAREN{}$  \BIND{} $e$.
...
The variables $p_i$ are added to $s$, with ranks $R_i, i \in 1..n$ respectively.
...
The function $makeTensor(dims: \ATSIGN{}, contents: [])$
...
ShapeRank supports a number of higher-order functions. They treat their function-valued arguments as scalar streams [\ref{scalars}]. We give the signatures of these functions as we do those of  ShapeRank programs, but extended with the notation for function types.
...
A program signature specifies the name of the program as whole. In addition it specifies the program's parameters.  These include {\em value parameters} and {\em type parameters}. 
...
Each value parameter has a specification similar to that of a function parameter. However, rather than specifying a rank for each parameter, a shape and base type are specified. The shape may be elided. Unlike a function, a program is not invoked by a function call [\ref{calls}]. Instead, it is invoked from the surrounding computing environment in an implementation specific manner.  The invocation binds the type parameters \ref{typeVariables} and value parameters of the program. 
...
The type of $p$ is the type of $e$.
...
Given hyperstreams $h_i$, each with dynamic size $s_i$ and $n_i$  available elements, $ i \in 1..k, k > 0$:
...
Of necessity, the number of available elements of $h_{sync}$ at any time is no more than the minimum among the $n_i$.
...
An {\em integer literal} represents an integer scalar.  An integer literal with value $n$ have type $[]n{}$ in any scope $S$.
...
Let $T_e$ be the type of $e$ is scope $S$.  It is a compile-time error if the base type of $T_e$ is not a struct type with a member named $p$. 
...
A {\em unary operator} is one of the operators \MINUS{} or  \NOT{}. The \MINUS{} operator implements IEEE floating point negation, with type $T \LT: \NUM{} \to T$. The \NOT{} operator implements boolean negation with type $\BOOL{} \to \BOOL$.
...
The expression $e_1 \theta e_2$ where $\theta$ is an additive operator, is equivalent to $\theta\LPAREN{}e_1\COMMA{} e_2\RPAREN$, where the signature of $\theta$ is $\theta\LPAREN{}T \LT: \NUM\COMMA{}T\RPAREN\COLON{} T$.
...
The expression $e_1 \theta e_2$ where $\theta$ is a relational operator, is equivalent to $\theta\LPAREN{}e_1\COMMA{} e_2\RPAREN$. The type of a relational expression \BOOL{} in any scope $S$.
...
\item  as an argument to a higher-order function whose base type is a function.
\item as the value of a let binding, or 
\item immediately following the keyword \RERANK{} another rerank expression. 
\end{itemize}
...
\subsubsection{Hyperstream Descriptors}
\label{hyperstreamDescriptors}
...
\paragraph{Scalar Type Descriptors.}
 A scalar type descriptor is either a type variable [\ref{typeVariables}] or one of \BOOL [\ref{bool}],  \NUM,[\ref{number}] \INT, \FLOAT, \STRING [\ref{string}], an integer literal \ref{integerSingletonTypes}, or a struct type [\ref{structTypes}].
